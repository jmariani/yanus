 ...
 */
class YFont extends CApplicationComponent {
    // Bootstrap plugins.
    ...
    public function init() {
        ...
        if (Yii::getPathOfAlias('yfont') === false)
            Yii::setPathOfAlias('yfont', realpath(dirname(__FILE__) . '/..'));


        // Prevents the extension from registering scripts and publishing assets when ran from the command line.
        if (Yii::app() instanceof CConsoleApplication)
            return;
        Yii::app()->getClientScript()->registerCssFile($this->getAssetsUrl() . "/style.css"); //, $media);
...
//            $this->registerCoreScripts();
    ...
    public function getAssetsUrl() {
        if (isset($this->_assetsUrl))
            ...
            $assetsPath = Yii::getPathOfAlias('yfont.assets');
