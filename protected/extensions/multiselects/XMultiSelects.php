...
class XMultiSelects extends CWidget
{
	...
	public function init()
	{
		...
		if(!isset($this->id))
		{
			throw new CHttpException(500,'"id" have to be set!');
		}
	}
	...
	public function run()
	{
		...
		echo "<select name=\"{$this->leftName}\" id=\"{$this->id}_select_left\" multiple=\"multiple\" size=\"{$this->size}\" style=\"width:{$this->width}\">\n";
		...
		echo "<input type=\"button\" style=\"width:40px\" id=\"{$this->id}_options_left\" value=\"&lt;\" /><br /><br />\n";
		echo "<input type=\"button\" style=\"width:40px\" id=\"{$this->id}_options_right\" value=\"&gt;\" /><br /><br />\n";
		echo "<input type=\"button\" style=\"width:40px\" id=\"{$this->id}_options_left_all\" value=\"&lt;&lt;\" /><br /><br />\n";
		echo "<input type=\"button\" style=\"width:40px\" id=\"{$this->id}_options_right_all\" value=\"&gt;&gt;\" /><br /><br /></td>\n";
		...
		echo "<select name=\"{$this->rightName}\" id=\"{$this->id}_select_right\" multiple=\"multiple\" size=\"{$this->size}\" style=\"width:{$this->width}\">\n";
		...
		echo "\$(function() {\n";
		echo "\$(\"#{$this->id}_select_left\").multiSelect(\"#{$this->id}_select_right\", {trigger: \"#{$this->id}_options_right\"});\n";
		echo "\$(\"#{$this->id}_select_right\").multiSelect(\"#{$this->id}_select_left\", {trigger: \"#{$this->id}_options_left\"});\n";
		echo "\$(\"#{$this->id}_select_left\").multiSelect(\"#{$this->id}_select_right\", {allTrigger:\"#{$this->id}_options_right_all\"});\n";
		echo "\$(\"#{$this->id}_select_right\").multiSelect(\"#{$this->id}_select_left\", {allTrigger:\"#{$this->id}_options_left_all\"});\n";
		echo "});\n";
		echo "// --></script>\n";
		parent::init();
	}
